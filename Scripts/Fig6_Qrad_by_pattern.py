...
# -*- coding: utf-8 -*-
"""
plot environmental variables, radiative heating rates, and standard deviation of these heating rates for four example days (one for each pattern)
...
def plot_mean_environmental_profiles(xr_fish, xr_flower, xr_gravel, xr_sugar):
    ...
    for k in range(3):
        ...
        ax[k].tick_params(
            direction="in", bottom=True, top=True, left=True, right=True, grid_alpha=0.6
        )
    ...
    ax[0].plot(
        var1.mean(dim="launch_time"), km_vec, linewidth=5, color="blue", label=Dates[0]
    )
    ax[0].plot(
        var2.mean(dim="launch_time"),
        km_vec,
        linewidth=5,
        color="palevioletred",
        label=Dates[1],
...
def plot_rad_mean_profiles(rad_fish, rad_flower, rad_gravel, rad_sugar):
    ...
    fish_var = rad_fish[var].where(
        rad_fish["Q_rad_sw"].mean(dim="height") > 0, drop=True
    )
    flower_var = rad_flower[var].where(
        rad_flower["Q_rad_sw"].mean(dim="height") > 0, drop=True
    )
    gravel_var = rad_gravel[var].where(
        rad_gravel["Q_rad_sw"].mean(dim="height") > 0, drop=True
    )
    sugar_var = rad_sugar[var].where(
        rad_sugar["Q_rad_sw"].mean(dim="height") > 0, drop=True
    )
    km_vec = sugar_var.height.values / 1000

    ax[0].plot(
        fish_var.mean(dim="launch_time"),
        km_vec,
        linewidth=5,
        color="blue",
        label=Dates[0],
    ...
    ax[0].plot(
        flower_var.mean(dim="launch_time"),
        km_vec,
        linewidth=5,
        color="palevioletred",
        label=Dates[1],
    ...
    ax[0].plot(
        gravel_var.mean(dim="launch_time"),
        km_vec,
        linewidth=5,
        color="deepskyblue",
        label=Dates[2],
    ...
    ax[0].plot(
        sugar_var.mean(dim="launch_time"),
        km_vec,
        linewidth=5,
        color="lightgrey",
        label=Dates[3],
...
def plot_radiative_sigma(rad_fish, rad_flower, rad_gravel, rad_sugar):
    ...
    ax[0].plot(
        sugar_spatial_variance, km_vec, linewidth=5, color="lightgrey", label=Dates[3]
    )
